...
void FIMGAimAssistTarget::ResetTarget()
{
...
FRotator FIMGAimAssistTarget::GetRotationFromMovement(const FAimAssistOwnerViewData& OwnerInfo) const
{
...
float FIMGAimAssistTarget::CalculateRotationToTarget2D(float TargetX, float TargetY, float OffsetY) const
{
	if (TargetX <= 0.0f)
	{
		return 0.0f;
...
void FAimAssistOwnerViewData::UpdateViewData(const APlayerController* PC)
{
	...
	// Set the Team ID
	if (AIMGPlayerState* IMGPS = PlayerController->GetPlayerState<AIMGPlayerState>())
	{
		TeamID = IMGPS->GetTeamId();
	}
	else
	{
		TeamID = INDEX_NONE;
...
FInputActionValue UAimAssistInputModifier::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
...
#if ENABLE_DRAW_DEBUG
	if (IMGConsoleVariables::bDrawAimAssistDebug)
	{
	...
	if (!IMGConsoleVariables::bEnableAimAssist)
	{
		return CurrentValue;
	}
...
void UAimAssistInputModifier::UpdateTargetData(float DeltaTime)
{
	...
	SwapTargetCaches();
	const TArray<FIMGAimAssistTarget>& OldTargetCache = GetPreviousTargetCache();
	TArray<FIMGAimAssistTarget>& NewTargetCache = GetCurrentTargetCache();
	...
	for (FIMGAimAssistTarget& Target : NewTargetCache)
	{
...
const float UAimAssistInputModifier::GetSensitivtyScalar(const UIMGSettingsShared* SharedSettings) const
{
	if (SharedSettings && SensitivityLevelTable)
	{
		const EIMGGamepadSensitivity Sens = TargetingType == EIMGTargetingType::Normal ? SharedSettings->GetGamepadLookSensitivityPreset() : SharedSettings->GetGamepadTargetingSensitivityPreset();
	...
	return (TargetingType == EIMGTargetingType::Normal) ? 1.0f : 0.5f;	
}
...
FRotator UAimAssistInputModifier::UpdateRotationalVelocity(APlayerController* PC, float DeltaTime, FVector CurrentLookInputValue, FVector CurrentMoveInputValue)
{
	...
	const TArray<FIMGAimAssistTarget>& TargetCache = GetCurrentTargetCache();

	float LookStickDeadzone = 0.25f;
	float MoveStickDeadzone = 0.25f;
	float SettingStrengthScalar = (TargetingType == EIMGTargetingType::Normal) ? 1.0f : 0.5f;

	if (UIMGLocalPlayer* LP = Cast<UIMGLocalPlayer>(PC->GetLocalPlayer()))
	{
		UIMGSettingsShared* SharedSettings = LP->GetSharedSettings();
	...
	for (const FIMGAimAssistTarget& Target : TargetCache)
	{
		if (Target.bUnderAssistOuterReticle && Target.bIsVisible)
		{
...
void UAimAssistInputModifier::CalculateTargetStrengths(const FIMGAimAssistTarget& Target, float& OutPullStrength, float& OutSlowStrength) const
{
	const bool bIsADS = (TargetingType == EIMGTargetingType::ADS);
...
{
	if (!Canvas || !OwnerViewData.IsDataValid() || !IMGConsoleVariables::bDrawAimAssistDebug)
	{
		return;
	}
