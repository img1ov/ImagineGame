...
const FIMGAimAssistTarget* FindTarget(const TArray<FIMGAimAssistTarget>& Targets, const UShapeComponent* TargetComponent)
{
	const FIMGAimAssistTarget* FoundTarget = Targets.FindByPredicate(
	[&TargetComponent](const FIMGAimAssistTarget& Target)
	{
...
void UAimAssistTargetManagerComponent::GetVisibleTargets(const FAimAssistFilter& Filter, const FAimAssistSettings& Settings, const FAimAssistOwnerViewData& OwnerData, const TArray<FIMGAimAssistTarget>& OldTargets, OUT TArray<FIMGAimAssistTarget>& OutNewTargets)
{
	...
	// Do a world trace on the Aim Assist channel to get any visible targets
	{
		...
		if(IMGConsoleVariables::bDrawDebugViewfinder)
		{
	...
	// Gather targets that are in front of the player
	{
		...
		for (FAimAssistTargetOptions& AimAssistTarget : NewTargetData)
		{
			...
			const FIMGAimAssistTarget* OldTarget = FindTarget(OldTargets, AimAssistTarget.TargetShapeComponent.Get());
	...
	{
		OutNewTargets.Sort([](const FIMGAimAssistTarget& TargetA, const FIMGAimAssistTarget& TargetB)
		{
	...
	{
		for (FIMGAimAssistTarget& Target : OutNewTargets)
		{
...
bool UAimAssistTargetManagerComponent::DoesTargetPassFilter(const FAimAssistOwnerViewData& OwnerData, const FAimAssistFilter& Filter, const FAimAssistTargetOptions& Target, const float AcceptableRange) const
{
	...
	if (const ACharacter* TargetCharacter = Cast<ACharacter>(TargetOwningActor))
	{
		...
		{
			if (const AIMGPlayerState* PS = TargetCharacter->GetPlayerState<AIMGPlayerState>())
			{
		...
		{
			if (const UIMGHealthComponent* HealthComponent = UIMGHealthComponent::FindHealthComponent(TargetCharacter))
			{
...
void UAimAssistTargetManagerComponent::DetermineTargetVisibility(FIMGAimAssistTarget& Target, const FAimAssistSettings& Settings, const FAimAssistFilter& Filter, const FAimAssistOwnerViewData& OwnerData)
{
	UWorld* World = GetWorld();
	check(World);
