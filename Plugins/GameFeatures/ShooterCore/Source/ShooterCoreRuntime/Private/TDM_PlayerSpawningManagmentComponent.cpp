...
AActor* UTDM_PlayerSpawningManagmentComponent::OnChoosePlayerStart(AController* Player, TArray<AIMGPlayerStart*>& PlayerStarts)
{
	...
	AIMGGameState* GameState = GetGameStateChecked<AIMGGameState>();

	AIMGPlayerStart* BestPlayerStart = nullptr;
	double MaxDistance = 0;
	AIMGPlayerStart* FallbackPlayerStart = nullptr;
	...
	for (APlayerState* PS : GameState->PlayerArray)
	{
		...
		{
			for (AIMGPlayerStart* PlayerStart : PlayerStarts)
			{
				if (APawn* Pawn = PS->GetPawn())
				{
					...
					else if (PlayerStart->GetLocationOccupancy(Player) < EIMGPlayerStartLocationOccupancy::Full)
					{
