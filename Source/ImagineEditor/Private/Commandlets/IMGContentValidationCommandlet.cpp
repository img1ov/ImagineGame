...
#include "Commandlets/IMGContentValidationCommandlet.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGContentValidationCommandlet)

DEFINE_LOG_CATEGORY_STATIC(LogIMGContentValidation, Log, Log);

class FIMGScopedContentValidationMessageGatherer : public FOutputDevice
{
public:

	FIMGScopedContentValidationMessageGatherer()
...
UIMGContentValidationCommandlet::UIMGContentValidationCommandlet(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

int32 UIMGContentValidationCommandlet::Main(const FString& FullCommandLine)
{
	UE_LOG(LogIMGContentValidation, Display, TEXT("Running IMGContentValidationCommandlet..."));
	...
	if (P4FilterString && !P4FilterString->IsEmpty())
	{
		...
		{
			UE_LOG(LogIMGContentValidation, Display, TEXT("ContentValidation returning 1. Failed to get changed files."));
			ReturnVal = 1;
		}
	}
	...
	if (bP4Opened)
	{
		...
		{
			UE_LOG(LogIMGContentValidation, Error, TEXT("P4 workspace was not found when using P4Opened"));
			UE_LOG(LogIMGContentValidation, Display, TEXT("ContentValidation returning 1. Workspace not found."));
			ReturnVal = 1;
		}
	}
	...
	UIMGEditorValidator::ValidatePackages(ChangedPackageNames, DeletedPackageNames, MaxPackagesToLoad, AllWarningsAndErrors, EDataValidationUsecase::Commandlet);

	if (!UIMGEditorValidator::ValidateProjectSettings())
	{
		ReturnVal = 1;
	}
...
bool UIMGContentValidationCommandlet::GetAllChangedFiles(IAssetRegistry& AssetRegistry, const FString& P4CmdString, TArray<FString>& OutChangedPackageNames, TArray<FString>& DeletedPackageNames, TArray<FString>& OutChangedCode, TArray<FString>& OutChangedOtherFiles) const
{
	...
	if (LaunchP4(P4CmdString, Results, ReturnCode))
	{
				...
				if (Result.Split(TEXT("#"), &DepotPathName, &ExtraInfoAfterPound))
				{
					if (DepotPathName.EndsWith(TEXT(".uasset")) || DepotPathName.EndsWith(TEXT(".umap")))
					{
						FString FullPackageName;
						{
							...
							if (DepotPathName.Split(TEXT("ImagineGame/Content/"), nullptr, &PostContentPath)) //@TODO: RENAME: Potential issue when modules are renamed
							{
								if (!PostContentPath.IsEmpty())
								{
									...
									if (!UIMGEditorValidator::IsInUncookedFolder(PackageNameToTest))
									{
						...
						if (FullPackageName.IsEmpty())
						{
							...
							if (DepotPathName.Split(TEXT("ImagineGame/Plugins/"), nullptr, &PostPluginsPath))
							{
					...
					{
						FString PostImagineGamePath;
						if (DepotPathName.Split(TEXT("/ImagineGame/"), nullptr, &PostImagineGamePath))
						{
							if (DepotPathName.EndsWith(TEXT(".cpp")))
							{
								OutChangedCode.Add(PostImagineGamePath);
							}
							else if (DepotPathName.EndsWith(TEXT(".h")))
							{
								...
								{
									UIMGEditorValidator::GetChangedAssetsForCode(AssetRegistry, ChangedHeaderLocalFilename, OutChangedPackageNames);
								}
							}
							else
							{
								OutChangedOtherFiles.Add(PostImagineGamePath);
							}
						}
					}
				}
		...
		{
			UE_LOG(LogIMGContentValidation, Error, TEXT("p4 returned non-zero return code %d"), ReturnCode);
		}
	}
...
void UIMGContentValidationCommandlet::GetAllPackagesInPath(IAssetRegistry& AssetRegistry, const FString& InPathString, TArray<FString>& OutPackageNames) const
{
...
void UIMGContentValidationCommandlet::GetAllPackagesOfType(const FString& OfTypeString, TArray<FString>& OutPackageNames) const
{
	...
	{
		FTopLevelAssetPath TypePathName = UClass::TryConvertShortTypeNameToPathName<UStruct>(Type, ELogVerbosity::Error, TEXT("UIMGContentValidationCommandlet"));
		if (TypePathName.IsNull())
		{
			UE_LOG(LogIMGContentValidation, Error, TEXT("Failed to convert short class name \"%s\" to path name. Please use class path names."), *Type);
		}
		else
		{
...
bool UIMGContentValidationCommandlet::LaunchP4(const FString& Args, TArray<FString>& Output, int32& OutReturnCode) const
{
	...
	{
		UE_LOG(LogIMGContentValidation, Error, TEXT("Failed to launch p4."));
	}
...
FString UIMGContentValidationCommandlet::GetLocalPathFromDepotPath(const FString& DepotPathName) const
{
	...
	if (!Workspace.IsEmpty())
	{
		...
		if (LaunchP4(P4WhereCommand, WhereResults, ReturnCode))
		{
			...
			{
				UE_LOG(LogIMGContentValidation, Warning, TEXT("GetAllChangedFiles failed to run p4 'where'. WhereResults[0] = '%s'. Not adding any validation for %s"), WhereResults.Num() > 0 ? *WhereResults[0] : TEXT("Invalid"), *DepotPathName);
			}
		}
	}
