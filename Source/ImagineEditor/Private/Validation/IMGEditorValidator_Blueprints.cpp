...
#include "Validation/IMGEditorValidator_Blueprints.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGEditorValidator_Blueprints)

#define LOCTEXT_NAMESPACE "EditorValidator"

UIMGEditorValidator_Blueprints::UIMGEditorValidator_Blueprints()
	: Super()
{
}

bool UIMGEditorValidator_Blueprints::CanValidateAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& InContext) const
{
	return Super::CanValidateAsset_Implementation(InAsset) && (InAsset ? InAsset->IsA(UBlueprint::StaticClass()) : false);
}

EDataValidationResult UIMGEditorValidator_Blueprints::ValidateLoadedAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& Context)
{
					...
					for (const FAssetData& ReferencerAssetData : ReferencerAssets)
					{
						...
						if ((ReferencerAssetData.PackageFlags & PKG_ContainsMap) == 0)
						{
							...
							if (ReferencerAssetData.GetTagValue<bool>(FBlueprintTags::IsDataOnly, bIsDataOnlyBP))
							{
								if (!bIsDataOnlyBP)
								{
									UE_LOG(LogImagineEditor, Display, TEXT("    Loading referencing non-dataonly blueprint %s"), *HardReferencerStr);
