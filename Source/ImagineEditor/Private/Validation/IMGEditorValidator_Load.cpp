...
#include "Validation/IMGEditorValidator_Load.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGEditorValidator_Load)
...
TArray<FString> UIMGEditorValidator_Load::InMemoryReloadLogIgnoreList = { TEXT("Enum name collision: '") };

UIMGEditorValidator_Load::UIMGEditorValidator_Load()
	: Super()
{
}

bool UIMGEditorValidator_Load::IsEnabled() const
{
...
bool UIMGEditorValidator_Load::CanValidateAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& InContext) const
{
	return Super::CanValidateAsset_Implementation(InAsset) && InAsset != nullptr;
}

EDataValidationResult UIMGEditorValidator_Load::ValidateLoadedAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& Context)
{
	check(InAsset);
...
bool UIMGEditorValidator_Load::GetLoadWarningsAndErrorsForPackage(const FString& PackageName, TArray<FString>& OutWarningsAndErrors)
{
	...
	if (ExistingPackage && !IsRunningCommandlet() && UIMGEditorValidator::ShouldAllowFullValidation() && !ExistingPackage->ContainsMap() && !PackageName.EndsWith(TEXT("_BuiltData")))
	{
		...
		if (bSourceFileExists)
		{
			...
			if (ensure(CopyResult == COPY_OK))
			{
				...
				{
					FIMGValidationMessageGatherer::AddIgnorePatterns(InMemoryReloadLogIgnoreList);
					...
					FIMGValidationMessageGatherer::RemoveIgnorePatterns(InMemoryReloadLogIgnoreList);
				}
				if (LoadedPackage)
				{
