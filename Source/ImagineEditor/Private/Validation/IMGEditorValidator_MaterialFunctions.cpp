...
#include "Validation/IMGEditorValidator_MaterialFunctions.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGEditorValidator_MaterialFunctions)

#define LOCTEXT_NAMESPACE "EditorValidator"

UIMGEditorValidator_MaterialFunctions::UIMGEditorValidator_MaterialFunctions()
	: Super()
{
}

bool UIMGEditorValidator_MaterialFunctions::CanValidateAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& InContext) const
{
	return (InAsset ? InAsset->IsA(UMaterialFunction::StaticClass()) : false);
}

EDataValidationResult UIMGEditorValidator_MaterialFunctions::ValidateLoadedAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& Context)
{
	UMaterialFunction* MaterialFunction = Cast<UMaterialFunction>(InAsset);
	check(MaterialFunction);

	if (UIMGEditorValidator::ShouldAllowFullValidation())
	{
		...
		for (FName HardReferencer : AllHardReferencers)
		{
			...
			if (!IsInUncookedFolder(HardReferencerStr))
			{
				...
				for (const FAssetData& ReferencerAssetData : ReferencerAssets)
				{
					...
					{
						UE_LOG(LogImagineEditor, Display, TEXT("    Loading referencing material %s"), *HardReferencerStr);

						TArray<FString> WarningsAndErrors;
						if (UIMGEditorValidator_Load::GetLoadWarningsAndErrorsForPackage(HardReferencerStr, WarningsAndErrors))
						{
