...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGEditorValidator)
...
bool UIMGEditorValidator::bAllowFullValidationInEditor = false;
TArray<FString> FIMGValidationMessageGatherer::IgnorePatterns;

UIMGEditorValidator::UIMGEditorValidator()
	: Super()
{
}

void UIMGEditorValidator::ValidateCheckedOutContent(bool bInteractive, const EDataValidationUsecase InValidationUsecase)
{
	...
	if (AssetRegistryModule.Get().IsLoadingAssets())
	{
		...
		{
			UE_LOG(LogImagineEditor, Display, TEXT("Could not run ValidateCheckedOutContent because asset discovery was still being done."));
		}
		return;
	}
	...
	if (ISourceControlModule::Get().IsEnabled())
	{
		...
		for (const FSourceControlStateRef& FileState : CheckedOutFiles)
		{
			...
			else if (Filename.EndsWith(TEXT(".h")))
			{
				...
				UIMGEditorValidator::GetChangedAssetsForCode(AssetRegistryModule.Get(), Filename, ChangedPackageNames);
			}
		}
	}
	...
	{
		FIMGValidationMessageGatherer ScopedMessageGatherer;
...
bool UIMGEditorValidator::ValidatePackages(const TArray<FString>& ExistingPackageNames, const TArray<FString>& DeletedPackageNames, int32 MaxPackagesToLoad, TArray<FString>& OutAllWarningsAndErrors, const EDataValidationUsecase InValidationUsecase)
{
	bool bAnyIssuesFound = false;
	...
	{
		UE_LOG(LogImagineEditor, Display, TEXT("Adding referencers for deleted package %s to be verified"), *DeletedPackageName);
		...
		for (const FName& Referencer : PackageReferencers)
		{
			...
			{
				UE_LOG(LogImagineEditor, Display, TEXT("    Deleted package referencer %s was added to the queue to be verified"), *ReferencerString);
	...
	if (AllPackagesToValidate.Num() > MaxPackagesToLoad)
	{
		...
		UE_LOG(LogImagineEditor, Warning, TEXT("%s"), *WarningMessage);
		...
		if (AssetsToCheck.Num() > 0)
		{
			// Preload all assets to check, so load warnings can be handled separately from validation warnings
			{
				for (const FAssetData& AssetToCheck : AssetsToCheck)
				{
					...
					{
						UE_LOG(LogImagineEditor, Display, TEXT("Preloading %s..."), *AssetToCheck.GetObjectPathString());

						// Start listening for load warnings
						FIMGValidationMessageGatherer ScopedPreloadMessageGatherer;
						...
						if (ScopedPreloadMessageGatherer.GetAllWarningsAndErrors().Num() > 0)
						{
							...
							{
								UE_LOG(LogImagineEditor, Error, TEXT("%s"), *LoadWarning);
							}
...
bool UIMGEditorValidator::ValidateProjectSettings()
{
	bool bSuccess = true;
...

	{
		...
		if (bDeveloperMode)
		{
			...
			UE_LOG(LogImagineEditor, Error, TEXT("%s"), *ErrorMessage);
...
bool UIMGEditorValidator::IsInUncookedFolder(const FString& PackageName, FString* OutUncookedFolderName)
{
...
bool UIMGEditorValidator::ShouldAllowFullValidation()
{
	return IsRunningCommandlet() || bAllowFullValidationInEditor;
}

bool UIMGEditorValidator::CanValidateAsset_Implementation(UObject* InAsset) const
{
	if (InAsset)
	{
...
void UIMGEditorValidator::GetChangedAssetsForCode(IAssetRegistry& AssetRegistry, const FString& ChangedHeaderLocalFilename, TArray<FString>& OutChangedPackageNames)
{
	...
	if (Module)
	{
		...
		for (TWeakObjectPtr<UClass> ModifiedClassPtr : ClassList)
		{
			...
			if (UClass* ModifiedClass = ModifiedClassPtr.Get())
			{
				...
				UE_LOG(LogImagineEditor, Display, TEXT("Validating Subclasses of %s in %s + %s"), *ModifiedClass->GetName(), *ChangedHeaderModule, *ChangedHeaderReleativeToModule);
				...
				AssetRegistry.EnumerateAssets(Filter, [&BlueprintsDerivedFromNativeModifiedClasses, &bTooManyFiles, &DerivedClassNames, ChangedHeaderModule, ChangedHeaderReleativeToModule](const FAssetData& AssetData)
					{
						...
						if (!AssetData.GetTagValueRef<bool>(FBlueprintTags::IsDataOnly) && !UIMGEditorValidator::IsInUncookedFolder(PackageName))
						{
							...
							if (!ClassFromData.IsEmpty())
							{
								...
								{
									UE_LOG(LogImagineEditor, Display, TEXT("\tAdding %s To Validate"), *PackageName);
									...
									{
										bTooManyFiles = true;
										UE_LOG(LogImagineEditor, Display, TEXT("Too many assets invalidated (Max %d) by change to, %s + %s"), GMaxAssetsChangedByAHeader, *ChangedHeaderModule, *ChangedHeaderReleativeToModule);
