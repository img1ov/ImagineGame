...
#include "Validation/IMGEditorValidator_SourceControl.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGEditorValidator_SourceControl)

#define LOCTEXT_NAMESPACE "EditorValidator"

UIMGEditorValidator_SourceControl::UIMGEditorValidator_SourceControl()
	: Super()
{
	
}

bool UIMGEditorValidator_SourceControl::CanValidateAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& InContext) const
{
	return InAsset != nullptr;
}

EDataValidationResult UIMGEditorValidator_SourceControl::ValidateLoadedAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset, FDataValidationContext& Context)
{
	check(InAsset);
