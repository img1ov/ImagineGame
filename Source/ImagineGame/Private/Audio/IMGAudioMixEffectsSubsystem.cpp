...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGAudioMixEffectsSubsystem)

class FSubsystemCollectionBase;

void UIMGAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void UIMGAudioMixEffectsSubsystem::Deinitialize()
{
...
bool UIMGAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UIMGAudioMixEffectsSubsystem::PostInitialize()
{
	Super::PostInitialize();

	if (const UIMGAudioSettings* IMGAudioSettings = GetDefault<UIMGAudioSettings>())
	{
		if (UObject* ObjPath = IMGAudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from IMG Audio Settings."));
			}
		}

		if (UObject* ObjPath = IMGAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from IMG Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = IMGAudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from IMG Audio Settings."));
			}
		}
		...
		for (const FIMGSubmixEffectChainMap& SoftSubmixEffectChain : IMGAudioSettings->HDRAudioSubmixEffectChain)
		{
			FIMGAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FIMGSubmixEffectChainMap& SoftSubmixEffectChain : IMGAudioSettings->LDRAudioSubmixEffectChain)
		{
			FIMGAudioSubmixEffectsChain NewEffectChain;
...
void UIMGAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	...
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const UIMGSettingsLocal* IMGSettingsLocal = GetDefault<UIMGSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, IMGSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, IMGSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, IMGSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, IMGSettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, IMGSettingsLocal->GetVoiceChatVolume());
			...
			ApplyDynamicRangeEffectsChains(IMGSettingsLocal->IsHDRAudioModeEnabled());
		}
	}
}

void UIMGAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FIMGAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FIMGAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FIMGAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FIMGAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void UIMGAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void UIMGAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool UIMGAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
