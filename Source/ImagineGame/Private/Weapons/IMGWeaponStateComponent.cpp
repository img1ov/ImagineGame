...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

UIMGWeaponStateComponent::UIMGWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UIMGWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (APawn* Pawn = GetPawn<APawn>())
	{
		...
		{
			if (UIMGRangedWeaponInstance* CurrentWeapon = Cast<UIMGRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(UIMGRangedWeaponInstance::StaticClass())))
			{
...
bool UIMGWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UIMGWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
...
void UIMGWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FIMGServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FIMGScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void UIMGWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UIMGWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UIMGWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
