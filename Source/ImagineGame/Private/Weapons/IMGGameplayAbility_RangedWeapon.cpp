...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGGameplayAbility_RangedWeapon)
...
namespace IMGConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("IMG.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("IMG.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("IMG.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
{
	return Cast<UIMGRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UIMGGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogIMGAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UIMGRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UIMGGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UIMGGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UIMGGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return IMG_TraceChannel_Weapon;
}

FHitResult UIMGGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UIMGGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UIMGGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EIMGAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UIMGWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UIMGWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EIMGAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EIMGAbilityTargetingSource::CameraTowardsFocus) || (Source == EIMGAbilityTargetingSource::PawnTowardsFocus) || (Source == EIMGAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EIMGAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EIMGAbilityTargetingSource::WeaponForward) || (Source == EIMGAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EIMGAbilityTargetingSource::PawnTowardsFocus) || (Source == EIMGAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UIMGGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (IMGConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, IMGConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UIMGGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());
	...
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EIMGAbilityTargetingSource::CameraTowardsFocus);
		...
		if (IMGConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, IMGConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UIMGGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UIMGRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (IMGConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, IMGConsoleVariables::DrawBulletHitRadius, FColor::Red, false, IMGConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UIMGGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UIMGGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UIMGGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		{
			UE_LOG(LogIMGAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UIMGGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UIMGWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UIMGWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FIMGGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FIMGGameplayAbilityTargetData_SingleTargetHit();
