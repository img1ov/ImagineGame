...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_IMG_Weapon_SteadyAimingCamera, "IMG.Weapon.SteadyAimingCamera");

UIMGRangedWeaponInstance::UIMGRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UIMGRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UIMGRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UIMGRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UIMGRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UIMGRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UIMGRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UIMGRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UIMGRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UIMGRangedWeaponInstance::AddSpread()
{
...
float UIMGRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UIMGRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UIMGRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UIMGRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	{
		float TopCameraWeight = 0.f;
		FGameplayTag TopCameraTag;
		CameraComponent->GetBlendInfo(/*out*/ TopCameraWeight, /*out*/ TopCameraTag);

		AimingAlpha = (TopCameraTag == TAG_IMG_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
