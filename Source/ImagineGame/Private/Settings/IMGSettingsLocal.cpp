...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("IMG.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("IMG.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("IMG.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("IMG.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("IMG.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("IMG.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("IMG.DeviceProfile.Mobile.MaxFrameRate"),
	120,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("IMG.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("IMG.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("IMG.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FIMGScalabilitySnapshot::FIMGScalabilitySnapshot()
{
...
namespace IMGSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FIMGScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UIMGSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UIMGSettingsLocal::UIMGSettingsLocal()
{
	...
	bEnableScalabilitySettings = UIMGPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings;

	SetToDefaults();
}

void UIMGSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
	...
	bEnableLatencyTrackingStats = UIMGSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void UIMGSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EIMGFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	IMGSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UIMGSettingsLocal::ResetToCurrentSettings()
{
...
void UIMGSettingsLocal::BeginDestroy()
{
...
UIMGSettingsLocal* UIMGSettingsLocal::Get()
{
	return GEngine ? CastChecked<UIMGSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UIMGSettingsLocal::ConfirmVideoMode()
{
...
float UIMGSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EIMGFramePacingMode::DesktopStyle)
	{
...
int32 UIMGSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return IMGSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UIMGSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FIMGScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UIMGSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UIMGSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UIMGSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UIMGSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UIMGSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UIMGSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EIMGStatDisplayMode UIMGSettingsLocal::GetPerfStatDisplayState(EIMGDisplayablePerformanceStat Stat) const
{
	if (const EIMGStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EIMGStatDisplayMode::Hidden;
	}
}

void UIMGSettingsLocal::SetPerfStatDisplayState(EIMGDisplayablePerformanceStat Stat, EIMGStatDisplayMode DisplayMode)
{
	if (DisplayMode == EIMGStatDisplayMode::Hidden)
	{
...
bool UIMGSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void UIMGSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void UIMGSettingsLocal::ApplyLatencyTrackingStatSetting()
{
...
bool UIMGSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float UIMGSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UIMGSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UIMGSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UIMGSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UIMGSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UIMGSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UIMGSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UIMGSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UIMGSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UIMGSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UIMGSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UIMGSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
float UIMGSettingsLocal::GetDynamicResolutionFrameRateTarget() const
{
	return DynamicResolutionFrameTarget;
}

void UIMGSettingsLocal::SetDynamicResolutionFrameRateTarget(float NewDynamicResolutionFPS)
{
	if (!IsRunningDedicatedServer())
	{
...
int32 UIMGSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UIMGSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UIMGSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UIMGSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return IMGSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UIMGSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return IMGSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UIMGSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UIMGSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EIMGFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return IMGSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UIMGSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EIMGFramePacingMode::MobileStyle)
	{
...
void UIMGSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UIMGSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EIMGFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = IMGSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UIMGSettingsLocal::ClampMobileQuality()
{
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EIMGFramePacingMode::MobileStyle)
	{
...
void UIMGSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = IMGSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UIMGSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = IMGSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = IMGSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UIMGSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UIMGSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UIMGSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UIMGSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UIMGSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = IMGSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UIMGSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UIMGSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
...
bool UIMGSettingsLocal::CanRunAutoBenchmark() const
{
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UIMGSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UIMGSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UIMGSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UIMGSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UIMGSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UIMGSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UIMGSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UIMGSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UIMGSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UIMGSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UIMGSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UIMGSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UIMGSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UIMGSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UIMGSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UIMGSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UIMGSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UIMGSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EIMGFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = IMGSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UIMGSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UIMGSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UIMGSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{/*
			if (const UIMGAudioSettings* IMGAudioSettings = GetDefault<UIMGAudioSettings>())
			{
				...
				if (UObject* ObjPath = IMGAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from IMG Audio Settings."));
					}
				}

				if (UObject* ObjPath = IMGAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from IMG Audio Settings."));
					}
				}

				if (UObject* ObjPath = IMGAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from IMG Audio Settings."));
					}
				}

				if (UObject* ObjPath = IMGAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from IMG Audio Settings."));
					}
				}

				if (UObject* ObjPath = IMGAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from IMG Audio Settings."));
					}
				}

				if (UObject* ObjPath = IMGAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from IMG Audio Settings."));
					}
				}
			}
		*/}
...
void UIMGSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UIMGSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
	...
	const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
	const TArray<FIMGQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FIMGQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UIMGPlatformEmulationSettings* Settings = GetDefault<UIMGPlatformEmulationSettings>();
	...
	{
	case EIMGFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EIMGFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EIMGFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UIMGSettingsLocal::UpdateConsoleFramePacing()
{
...
void UIMGSettingsLocal::UpdateDesktopFramePacing()
{
...
void UIMGSettingsLocal::UpdateMobileFramePacing()
{
...
void UIMGSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
