...
#include "Settings/Screens/IMGSafeZoneEditor.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGSafeZoneEditor)
...
UIMGSafeZoneEditor::UIMGSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UIMGSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UIMGSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UIMGSettingsLocal::Get()->GetSafeZone());

	Button_Done->OnClicked().AddUObject(this, &UIMGSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UIMGSafeZoneEditor::HandleBackClicked);
	}
}

bool UIMGSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UIMGSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UIMGSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UIMGSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UIMGSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UIMGSettingsLocal::Get()->GetSafeZone());
}

void UIMGSafeZoneEditor::HandleDoneClicked()
{
	...
	{
		UIMGSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
