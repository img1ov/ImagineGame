...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGSettingValueDiscrete_Resolution)

#define LOCTEXT_NAMESPACE "IMGSettings"

UIMGSettingValueDiscrete_Resolution::UIMGSettingValueDiscrete_Resolution()
{
}

void UIMGSettingValueDiscrete_Resolution::BeginDestroy()
{
	Super::BeginDestroy();
...
void UIMGSettingValueDiscrete_Resolution::OnInitialized()
{
	Super::OnInitialized();
	...
	if (ensure(PlatformApplication.IsValid()))
	{
		...
		{
			DisplayMetricsChangedHandle = DisplayMetricsChangedEvent.AddUObject(this, &UIMGSettingValueDiscrete_Resolution::OnDisplayMetricsChanged);
		}
	}
...
void UIMGSettingValueDiscrete_Resolution::StoreInitial()
{
	// Ignored
}

void UIMGSettingValueDiscrete_Resolution::ResetToDefault()
{
	// Ignored
}

void UIMGSettingValueDiscrete_Resolution::RestoreToInitial()
{
	// Ignored
}

void UIMGSettingValueDiscrete_Resolution::SetDiscreteOptionByIndex(int32 Index)
{
	TArrayView<const TSharedPtr<UIMGSettingValueDiscrete_Resolution::FScreenResolutionEntry>> Resolutions = GetSelectedResolutionList();
...
int32 UIMGSettingValueDiscrete_Resolution::GetDiscreteOptionIndex() const
{
...
TArray<FText> UIMGSettingValueDiscrete_Resolution::GetDiscreteOptions() const
{
...
void UIMGSettingValueDiscrete_Resolution::OnDependencyChanged()
{
	InitializeResolutions();
...
void UIMGSettingValueDiscrete_Resolution::OnDisplayMetricsChanged(const FDisplayMetrics& NewDisplayMetrics)
{
	CurrentDisplayMetrics = NewDisplayMetrics;
	InitializeResolutions();
}

const FMonitorInfo* UIMGSettingValueDiscrete_Resolution::GetCurrentMonitor() const
{
	const UGameUserSettings* const UserSettings = GEngine->GetGameUserSettings();
...
void UIMGSettingValueDiscrete_Resolution::InitializeResolutions()
{
...
TArrayView<const TSharedPtr<UIMGSettingValueDiscrete_Resolution::FScreenResolutionEntry>> UIMGSettingValueDiscrete_Resolution::GetSelectedResolutionList() const
{
	TArrayView<const TSharedPtr<UIMGSettingValueDiscrete_Resolution::FScreenResolutionEntry>> Result;
...
bool UIMGSettingValueDiscrete_Resolution::ShouldAllowFullScreenResolution(const FScreenResolutionRHI& SrcScreenRes, int32 FilterThreshold) const
{
...
int32 UIMGSettingValueDiscrete_Resolution::FindIndexOfDisplayResolution(const FIntPoint& InPoint) const
{
	TArrayView<const TSharedPtr<UIMGSettingValueDiscrete_Resolution::FScreenResolutionEntry>> Resolutions = GetSelectedResolutionList();
...
int32 UIMGSettingValueDiscrete_Resolution::FindClosestResolutionIndex(const FIntPoint& Resolution) const
{
	TArrayView<const TSharedPtr<UIMGSettingValueDiscrete_Resolution::FScreenResolutionEntry>> Resolutions = GetSelectedResolutionList();
	int32 Index = 0;
...
void UIMGSettingValueDiscrete_Resolution::GetStandardWindowResolutions(const FIntPoint& MinResolution, const FIntPoint& MaxResolution, float MinAspectRatio, TArray<FIntPoint>& OutResolutions)
{
...
FText UIMGSettingValueDiscrete_Resolution::FScreenResolutionEntry::GetDisplayText() const
{
