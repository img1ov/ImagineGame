...
#include "Settings/CustomSettings/IMGSettingValueDiscrete_Resolution.h"
...
{
public:
	FGameSettingEditCondition_FramePacingMode(EIMGFramePacingMode InDesiredMode, EFramePacingEditCondition InMatchMode = EFramePacingEditCondition::EnableIf)
	...
	{
		const EIMGFramePacingMode ActualMode = UIMGPlatformSpecificRenderingSettings::Get()->FramePacingMode;
...
private:
	EIMGFramePacingMode DesiredMode;
...
{
public:
	FGameSettingEditCondition_DynamicResolution(EIMGFramePacingMode InDesiredMode, EFramePacingEditCondition InMatchMode = EFramePacingEditCondition::EnableIf)
		: DesiredMode(InDesiredMode)
		, MatchMode(InMatchMode)
	{
	}
...
public:
	...
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		if (!UIMGPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings)
		{
	...
	virtual void SettingChanged(const ULocalPlayer* LocalPlayer, UGameSetting* Setting, EGameSettingChangeReason Reason) const override
	{
		...
		const UIMGLocalPlayer* IMGLocalPlayer = CastChecked<UIMGLocalPlayer>(LocalPlayer);
		IMGLocalPlayer->GetLocalSettings()->ApplyScalabilitySettings();
	}
...
UGameSettingCollection* UIMGGameSettingRegistry::InitializeVideoSettings(UIMGLocalPlayer* InLocalPlayer)
{
	...
	UIMGSettingValueDiscrete_Display* DisplaySetting = nullptr;
#endif
	UIMGSettingValueDiscrete_Resolution* ResolutionSetting = nullptr;
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		{
#if SUPPORTS_DISPLAY_SETTING
			UIMGSettingValueDiscrete_Display* Setting = NewObject<UIMGSettingValueDiscrete_Display>();
		...
		//----------------------------------------------------------------------------------
		{
			UIMGSettingValueDiscrete_Resolution* Setting = NewObject<UIMGSettingValueDiscrete_Resolution>();
		...
		if (UGameInstance* GameInstance = InLocalPlayer->GetGameInstance())
		{
			...
			if (UIMGSettingsLocal* GameUserSettings = InLocalPlayer->GetLocalSettings()->Get())
			{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UIMGSettingsShared>()->GetColorBlindMode());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UIMGSettingsShared>()->GetColorBlindStrength());
...
#if PLATFORM_USES_DYNAMIC_HDR_SETTING
		{
			UIMGSettingAction_HDRCalibrationEditor* Setting = NewObject<UIMGSettingAction_HDRCalibrationEditor>();
		...
		//----------------------------------------------------------------------------------
		{
			UIMGSettingAction_SafeZoneEditor* Setting = NewObject<UIMGSettingAction_SafeZoneEditor>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const FIMGQualityDeviceProfileVariant& Variant : PlatformSettings->UserFacingDeviceProfileOptions)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			UIMGSettingValueDiscrete_MobileFPSType* Setting = NewObject<UIMGSettingValueDiscrete_MobileFPSType>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EIMGFramePacingMode::MobileStyle));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetCustomAction([](ULocalPlayer* LocalPlayer)
			{
				...
				{
					UIMGSettingsLocal::Get()->ResetToMobileDeviceDefaults();
				}
				else
				{
					const UIMGLocalPlayer* IMGLocalPlayer = CastChecked<UIMGLocalPlayer>(LocalPlayer);
					...
					IMGLocalPlayer->GetLocalSettings()->RunAutoBenchmark(bImmediatelySaveState);
				}
			});
			...
			{
				const UIMGPlatformSpecificRenderingSettings* PlatformSettings = UIMGPlatformSpecificRenderingSettings::Get();
				const bool bCanUseDueToMobile = (PlatformSettings->FramePacingMode == EIMGFramePacingMode::MobileStyle);

				const UIMGLocalPlayer* IMGLocalPlayer = CastChecked<UIMGLocalPlayer>(LocalPlayer);
				const bool bCanBenchmark = IMGLocalPlayer->GetLocalSettings()->CanRunAutoBenchmark();
		...
		//----------------------------------------------------------------------------------
		{
			UIMGSettingValueDiscrete_OverallQuality* Setting = NewObject<UIMGSettingValueDiscrete_OverallQuality>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EIMGFramePacingMode::ConsoleStyle, EFramePacingEditCondition::DisableIf));
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EIMGFramePacingMode::DesktopStyle));
...
void AddDynamicResolutionOptions(UGameSettingValueDiscreteDynamic_Number* Setting)
{
	...
	for (int32 Rate : GetDefault<UIMGPerformanceSettings>()->DesktopFrameRateLimits)
	{
...
void UIMGGameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, UIMGLocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		UGameSettingValueDiscreteDynamic_Number* Setting = NewObject<UGameSettingValueDiscreteDynamic_Number>();
		...
		Setting->SetDefaultValue(GetDefault<UIMGSettingsLocal>()->GetFrameRateLimit_OnBattery());
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UIMGSettingsLocal>()->GetFrameRateLimit_InMenu());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EIMGFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UIMGSettingsLocal>()->GetFrameRateLimit_WhenBackgrounded());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EIMGFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UIMGSettingsLocal>()->GetFrameRateLimit_Always());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EIMGFramePacingMode::DesktopStyle));
...

	{
		...
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_DynamicResolution>(EIMGFramePacingMode::DesktopStyle));
