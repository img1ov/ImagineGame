...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGPlayerInput)

UIMGPlayerInput::UIMGPlayerInput()
	: Super()
{
...
bool UIMGPlayerInput::InputKey(const FInputKeyEventArgs& Params)
{
	const bool bResult = Super::InputKey(Params);

	// Note: Since IMG is only going to support the "Reflex" plugin to handle latency markers,
...
void UIMGPlayerInput::ProcessInputEventForLatencyMarker(const FInputKeyEventArgs& Params)
{
...
void UIMGPlayerInput::BindToLatencyMarkerSettingChange()
{
...
void UIMGPlayerInput::UnbindLatencyMarkerSettingChangeListener()
{
	UIMGSettingsLocal* Settings = UIMGSettingsLocal::Get();
	if (!Settings)
	{
...
void UIMGPlayerInput::HandleLatencyMarkerSettingChanged()
{
	// Make sure that we only ever get this callback on platforms which support latency markers
	ensure(UIMGSettingsLocal::DoesPlatformSupportLatencyMarkers());
