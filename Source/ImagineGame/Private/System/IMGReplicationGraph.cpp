...
/**
*
*	===================== IMGReplicationGraph Replication =====================
*
*	Overview
*
*		This changes the way actor relevancy works. AActor::IsNetRelevantFor is NOT used in this system!
*
*		Instead, The UIMGReplicationGraph contains UReplicationGraphNodes. These nodes are responsible for generating lists of actors to replicate for each connection.
...
*
*		UIMGReplicationGraph is intended to not be directly used by the game code. That is, you should not have to include IMGReplicationGraph.h anywhere else.
*		Rather, UIMGReplicationGraph depends on the game code and registers for events that the game code broadcasts (e.g., events for players joining/leaving teams).
*		This choice was made because it gives UIMGReplicationGraph a complete holistic view of actor replication. Rather than exposing generic public functions that any
*		place in game code can invoke, all notifications are explicitly registered in UIMGReplicationGraph::InitGlobalActorClassSettings.
*
*	IMG Nodes
*
...
*
*		UIMGReplicationGraphNode_AlwaysRelevant_ForConnection
...
*
*		UIMGReplicationGraphNode_PlayerStateFrequencyLimiter
...
*		owning connection only) via UIMGReplicationGraphNode_AlwaysRelevant_ForConnection.
*
...
*
*		Making something always relevant to connection: You will need to modify UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection. You will also want
*		to make sure the actor does not get put in one of the other nodes. The safest way to do this is by setting its EClassRepNodeMapping to NotRouted in UIMGReplicationGraph::InitGlobalActorClassSettings.
*
*	How To Debug
*
...
*
*		IMG.RepGraph.PrintRouting - will print the EClassRepNodeMapping for each class. That is, how a given actor class is routed (or not) in the Replication Graph.
*
*/
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGReplicationGraph)

DEFINE_LOG_CATEGORY( LogIMGRepGraph );

namespace IMG::RepGraph
{
	float DestructionInfoMaxDist = 30000.f;
	static FAutoConsoleVariableRef CVarIMGRepGraphDestructMaxDist(TEXT("IMG.RepGraph.DestructInfo.MaxDist"), DestructionInfoMaxDist, TEXT("Max distance (not squared) to rep destruct infos at"), ECVF_Default);

	int32 DisplayClientLevelStreaming = 0;
	static FAutoConsoleVariableRef CVarIMGRepGraphDisplayClientLevelStreaming(TEXT("IMG.RepGraph.DisplayClientLevelStreaming"), DisplayClientLevelStreaming, TEXT(""), ECVF_Default);

	float CellSize = 10000.f;
	static FAutoConsoleVariableRef CVarIMGRepGraphCellSize(TEXT("IMG.RepGraph.CellSize"), CellSize, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarIMGRepGraphSpatialBiasX(TEXT("IMG.RepGraph.SpatialBiasX"), SpatialBiasX, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarIMGRepSpatialBiasY(TEXT("IMG.RepGraph.SpatialBiasY"), SpatialBiasY, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarIMGRepDynamicActorFrequencyBuckets(TEXT("IMG.RepGraph.DynamicActorFrequencyBuckets"), DynamicActorFrequencyBuckets, TEXT(""), ECVF_Default);

	int32 DisableSpatialRebuilds = 1;
	static FAutoConsoleVariableRef CVarIMGRepDisableSpatialRebuilds(TEXT("IMG.RepGraph.DisableSpatialRebuilds"), DisableSpatialRebuilds, TEXT(""), ECVF_Default);

	int32 LogLazyInitClasses = 0;
	static FAutoConsoleVariableRef CVarIMGRepLogLazyInitClasses(TEXT("IMG.RepGraph.LogLazyInitClasses"), LogLazyInitClasses, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarIMGRepTargetKBytesSecFastSharedPath(TEXT("IMG.RepGraph.TargetKBytesSecFastSharedPath"), TargetKBytesSecFastSharedPath, TEXT(""), ECVF_Default);

	float FastSharedPathCullDistPct = 0.80f;
	static FAutoConsoleVariableRef CVarIMGRepFastSharedPathCullDistPct(TEXT("IMG.RepGraph.FastSharedPathCullDistPct"), FastSharedPathCullDistPct, TEXT(""), ECVF_Default);

	int32 EnableFastSharedPath = 1;
	static FAutoConsoleVariableRef CVarIMGRepEnableFastSharedPath(TEXT("IMG.RepGraph.EnableFastSharedPath"), EnableFastSharedPath, TEXT(""), ECVF_Default);
	...
	UReplicationDriver* ConditionalCreateReplicationDriver(UNetDriver* ForNetDriver, UWorld* World)
	{
		...
		{
			const UIMGReplicationGraphSettings* IMGRepGraphSettings = GetDefault<UIMGReplicationGraphSettings>();

			// Enable/Disable via developer settings
			if (IMGRepGraphSettings && IMGRepGraphSettings->bDisableReplicationGraph)
			{
				UE_LOG(LogIMGRepGraph, Display, TEXT("Replication graph is disabled via IMGReplicationGraphSettings."));
				return nullptr;
			}

			UE_LOG(LogIMGRepGraph, Display, TEXT("Replication graph is enabled for %s in world %s."), *GetNameSafe(ForNetDriver), *GetPathNameSafe(World));

			TSubclassOf<UIMGReplicationGraph> GraphClass;
			if (IMGRepGraphSettings)
			{
				GraphClass = IMGRepGraphSettings->DefaultReplicationGraphClass.TryLoadClass<UIMGReplicationGraph>();
			}

			if (GraphClass.Get() == nullptr)
			{
				GraphClass = UIMGReplicationGraph::StaticClass();
			}

			UIMGReplicationGraph* IMGReplicationGraph = NewObject<UIMGReplicationGraph>(GetTransientPackage(), GraphClass.Get());
			return IMGReplicationGraph;
		}
...
UIMGReplicationGraph::UIMGReplicationGraph()
{
	if (!UReplicationDriver::CreateReplicationDriverDelegate().IsBound())
	{
		UReplicationDriver::CreateReplicationDriverDelegate().BindLambda(
			...
			{
				return IMG::RepGraph::ConditionalCreateReplicationDriver(ForNetDriver, World);
			});
	}
}

void UIMGReplicationGraph::ResetGameWorldState()
{
	...
	for (UNetReplicationGraphConnection* ConnManager : Connections)
	{
		...
		{
			if (UIMGReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = Cast<UIMGReplicationGraphNode_AlwaysRelevant_ForConnection>(ConnectionNode))
			{
...
EClassRepNodeMapping UIMGReplicationGraph::GetClassNodeMapping(UClass* Class) const
{
	if (!Class)
	{
...
void UIMGReplicationGraph::RegisterClassRepNodeMapping(UClass* Class)
{
	EClassRepNodeMapping Mapping = GetClassNodeMapping(Class);
	ClassRepNodePolicies.Set(Class, Mapping);
}

void UIMGReplicationGraph::InitClassReplicationInfo(FClassReplicationInfo& Info, UClass* Class, bool Spatialize) const
{
	AActor* CDO = Class->GetDefaultObject<AActor>();
	if (Spatialize)
	{
		Info.SetCullDistanceSquared(CDO->GetNetCullDistanceSquared());
		UE_LOG(LogIMGRepGraph, Log, TEXT("Setting cull distance for %s to %f (%f)"), *Class->GetName(), Info.GetCullDistanceSquared(), Info.GetCullDistance());
	}
	...
	UE_LOG(LogIMGRepGraph, Log, TEXT("Setting replication period for %s (%s) to %d frames (%.2f)"), *Class->GetName(), *NativeClass->GetName(), Info.ReplicationPeriodFrame, CDO->GetNetUpdateFrequency());
}

bool UIMGReplicationGraph::ConditionalInitClassReplicationInfo(UClass* ReplicatedClass, FClassReplicationInfo& ClassInfo)
{
...
void UIMGReplicationGraph::AddClassRepInfo(UClass* Class, EClassRepNodeMapping Mapping)
{
	if (IsSpatialized(Mapping))
	{
		if (Class->GetDefaultObject<AActor>()->bAlwaysRelevant)
		{
			UE_LOG(LogIMGRepGraph, Warning, TEXT("Replicated Class %s is AlwaysRelevant but is initialized into a spatialized node (%s)"), *Class->GetName(), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
		}
	}
...
void UIMGReplicationGraph::RegisterClassReplicationInfo(UClass* ReplicatedClass)
{
	...
	if (ConditionalInitClassReplicationInfo(ReplicatedClass, ClassInfo))
	{
		...
		UE_LOG(LogIMGRepGraph, Log, TEXT("Setting %s - %.2f"), *GetNameSafe(ReplicatedClass), ClassInfo.GetCullDistance());
	}
}

void UIMGReplicationGraph::InitGlobalActorClassSettings()
{
	...
	GlobalActorReplicationInfoMap.SetInitClassInfoFunc(
		[this](UClass* Class, FClassReplicationInfo& ClassInfo)
		{
			...
			if (IMG::RepGraph::LogLazyInitClasses != 0)
			{
				if (bHandled)
				{
					EClassRepNodeMapping Mapping = ClassRepNodePolicies.GetChecked(Class);
					UE_LOG(LogIMGRepGraph, Warning, TEXT("%s was Lazy Initialized. (Parent: %s) %d."), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()), (int32)Mapping);
					...
					{
						UE_LOG(LogIMGRepGraph, Warning, TEXT("Differences Found!"));
						FString DebugStr = ParentRepInfo.BuildDebugStringDelta();
						UE_LOG(LogIMGRepGraph, Warning, TEXT("  Parent: %s"), *DebugStr);

						DebugStr = ClassInfo.BuildDebugStringDelta();
						UE_LOG(LogIMGRepGraph, Warning, TEXT("  Class : %s"), *DebugStr);
					}
				}
				else
				{
					UE_LOG(LogIMGRepGraph, Warning, TEXT("%s skipped Lazy Initialization because it does not differ from its parent. (Parent: %s)"), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()));
	...
	const UIMGReplicationGraphSettings* IMGRepGraphSettings = GetDefault<UIMGReplicationGraphSettings>();
	check(IMGRepGraphSettings);

	// Set Classes Node Mappings
	for (const FRepGraphActorClassSettings& ActorClassSettings : IMGRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddClassRepInfoToMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(LogIMGRepGraph, Log, TEXT("ActorClassSettings -- AddClassRepInfo - %s :: %i"), *StaticActorClass->GetName(), int(ActorClassSettings.ClassNodeMapping));
...
#if WITH_GAMEPLAY_DEBUGGER
	AddClassRepInfo(AGameplayDebuggerCategoryReplicator::StaticClass(), EClassRepNodeMapping::NotRouted);				// Replicated via UIMGReplicationGraphNode_AlwaysRelevant_ForConnection
#endif
	...
	CharacterClassRepInfo.SetCullDistanceSquared(AIMGCharacter::StaticClass()->GetDefaultObject<AIMGCharacter>()->GetNetCullDistanceSquared());
...

	{
		...
		FRepMovement DefaultRepMovement = AIMGCharacter::StaticClass()->GetDefaultObject<AIMGCharacter>()->GetReplicatedMovement(); // Use the same quantization settings as our default replicatedmovement
	...
	{
		bool bSuccess = false;
		if (AIMGCharacter* Character = Cast<AIMGCharacter>(Actor))
		{
	...
	FastSharedPathConstants.MaxBitsPerFrame = (int32)((float)(IMG::RepGraph::TargetKBytesSecFastSharedPath * 1024 * 8) / NetDriver->GetNetServerMaxTickRate());
	FastSharedPathConstants.DistanceRequirementPct = IMG::RepGraph::FastSharedPathCullDistPct;

	SetClassInfo(AIMGCharacter::StaticClass(), CharacterClassRepInfo);

	// ---------------------------------------------------------------------
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.ListSize = 12;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.NumBuckets = IMG::RepGraph::DynamicActorFrequencyBuckets;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.BucketThresholds.Reset();
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.EnableFastPath = (IMG::RepGraph::EnableFastSharedPath > 0);
	...
	UE_LOG(LogIMGRepGraph, Log, TEXT(""));
	UE_LOG(LogIMGRepGraph, Log, TEXT("Class Routing Map: "));
	for (auto ClassMapIt = ClassRepNodePolicies.CreateIterator(); ClassMapIt; ++ClassMapIt)
	{
		...
		UE_LOG(LogIMGRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(ParentNativeClass), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
	}

	UE_LOG(LogIMGRepGraph, Log, TEXT(""));
	UE_LOG(LogIMGRepGraph, Log, TEXT("Class Settings Map: "));
	...
	for (auto ClassRepInfoIt = GlobalActorReplicationInfoMap.CreateClassMapIterator(); ClassRepInfoIt; ++ClassRepInfoIt)
	{
		...
		UE_LOG(LogIMGRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(GetParentNativeClass(Class)), *ClassInfo.BuildDebugStringDelta());
	}


	// Rep destruct infos based on CVar value
	DestructInfoMaxDistanceSquared = IMG::RepGraph::DestructionInfoMaxDist * IMG::RepGraph::DestructionInfoMaxDist;
	...
	for (const FRepGraphActorClassSettings& ActorClassSettings : IMGRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddToRPC_Multicast_OpenChannelForClassMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(LogIMGRepGraph, Log, TEXT("ActorClassSettings -- RPC_Multicast_OpenChannelForClass - %s"), *StaticActorClass->GetName());
...
void UIMGReplicationGraph::InitGlobalGraphNodes()
{
	// -----------------------------------------------
	//	Spatial Actors
	// -----------------------------------------------
	...
	GridNode->CellSize = IMG::RepGraph::CellSize;
	GridNode->SpatialBias = FVector2D(IMG::RepGraph::SpatialBiasX, IMG::RepGraph::SpatialBiasY);

	if (IMG::RepGraph::DisableSpatialRebuilds)
	{
	...
	// -----------------------------------------------
	UIMGReplicationGraphNode_PlayerStateFrequencyLimiter* PlayerStateNode = CreateNewNode<UIMGReplicationGraphNode_PlayerStateFrequencyLimiter>();
	AddGlobalGraphNode(PlayerStateNode);
}

void UIMGReplicationGraph::InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
	Super::InitConnectionGraphNodes(RepGraphConnection);

	UIMGReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = CreateNewNode<UIMGReplicationGraphNode_AlwaysRelevant_ForConnection>();

	// This node needs to know when client levels go in and out of visibility
	RepGraphConnection->OnClientVisibleLevelNameAdd.AddUObject(AlwaysRelevantConnectionNode, &UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd);
	RepGraphConnection->OnClientVisibleLevelNameRemove.AddUObject(AlwaysRelevantConnectionNode, &UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove);

	AddConnectionGraphNode(AlwaysRelevantConnectionNode, RepGraphConnection);
}

EClassRepNodeMapping UIMGReplicationGraph::GetMappingPolicy(UClass* Class)
{
...
void UIMGReplicationGraph::RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
...
void UIMGReplicationGraph::RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo)
{
	EClassRepNodeMapping Policy = GetMappingPolicy(ActorInfo.Class);
	switch(Policy)
	{
		...
		case EClassRepNodeMapping::RelevantAllConnections:
		{
			...
			else
			{
				...
				{
					UE_LOG(LogIMGRepGraph, Warning, TEXT("Actor %s was not found in AlwaysRelevantStreamingLevelActors list. LevelName: %s"), *GetActorRepListTypeDebugString(ActorInfo.Actor), *ActorInfo.StreamingLevelName.ToString());
				}
			}
...
#if WITH_GAMEPLAY_DEBUGGER
void UIMGReplicationGraph::OnGameplayDebuggerOwnerChange(AGameplayDebuggerCategoryReplicator* Debugger, APlayerController* OldOwner)
{
	CHECK_WORLDS(Debugger);

	auto GetAlwaysRelevantForConnectionNode = [this](APlayerController* Controller) -> UIMGReplicationGraphNode_AlwaysRelevant_ForConnection*
	{
		if (Controller)
		{
	...
	if (UIMGReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(OldOwner))
	{
		AlwaysRelevantConnectionNode->GameplayDebugger = nullptr;
	}

	if (UIMGReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(Debugger->GetReplicationOwner()))
	{
...
void UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::ResetGameWorldState()
{
	ReplicationActorList.Reset();
	AlwaysRelevantStreamingLevelsNeedingReplication.Empty();
}

void UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	UIMGReplicationGraph* IMGGraph = CastChecked<UIMGReplicationGraph>(GetOuter());
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (AIMGPlayerController* PC = Cast<AIMGPlayerController>(CurViewer.InViewer))
		{
			...
			if (bReplicatePS)
			{
				// Always return the player state to the owning player. Simulated proxy player states are handled by UIMGReplicationGraphNode_PlayerStateFrequencyLimiter
			...
			if (AIMGCharacter* Pawn = Cast<AIMGCharacter>(PC->GetPawn()))
			{
			...
			if (AIMGCharacter* ViewTargetPawn = Cast<AIMGCharacter>(CurViewer.ViewTarget))
			{
	...
	TMap<FName, FActorRepListRefView>& AlwaysRelevantStreamingLevelActors = IMGGraph->AlwaysRelevantStreamingLevelActors;
	...
	for (int32 Idx=AlwaysRelevantStreamingLevelsNeedingReplication.Num()-1; Idx >= 0; --Idx)
	{
		...
		if (Ptr == nullptr)
		{
			...
			UE_CLOG(IMG::RepGraph::DisplayClientLevelStreaming > 0, LogIMGRepGraph, Display, TEXT("CLIENTSTREAMING Removing %s from AlwaysRelevantStreamingLevelActors because FActorRepListRefView is null. %s "), *StreamingLevel.ToString(),  *Params.ConnectionManager.GetName());
		...
		if (RepList.Num() > 0)
		{
			...
			if (bAllDormant)
			{
				UE_CLOG(IMG::RepGraph::DisplayClientLevelStreaming > 0, LogIMGRepGraph, Display, TEXT("CLIENTSTREAMING All AlwaysRelevant Actors Dormant on StreamingLevel %s for %s. Removing list."), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				AlwaysRelevantStreamingLevelsNeedingReplication.RemoveAtSwap(Idx, EAllowShrinking::No);
			}
			else
			{
				UE_CLOG(IMG::RepGraph::DisplayClientLevelStreaming > 0, LogIMGRepGraph, Display, TEXT("CLIENTSTREAMING Adding always Actors on StreamingLevel %s for %s because it has at least one non dormant actor"), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				Params.OutGatheredReplicationLists.AddReplicationActorList(RepList);
			}
		}
		else
		{
			UE_LOG(LogIMGRepGraph, Warning, TEXT("UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection - empty RepList %s"), *Params.ConnectionManager.GetName());
		}
...
void UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd(FName LevelName, UWorld* StreamingWorld)
{
	UE_CLOG(IMG::RepGraph::DisplayClientLevelStreaming > 0, LogIMGRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityAdd - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Add(LevelName);
}

void UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove(FName LevelName)
{
	UE_CLOG(IMG::RepGraph::DisplayClientLevelStreaming > 0, LogIMGRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityRemove - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Remove(LevelName);
}

void UIMGReplicationGraphNode_AlwaysRelevant_ForConnection::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	...
	{
		UIMGReplicationGraph* IMGGraph = CastChecked<UIMGReplicationGraph>(GetOuter());
		if (FActorRepListRefView* RepList = IMGGraph->AlwaysRelevantStreamingLevelActors.Find(LevelName))
		{
...
UIMGReplicationGraphNode_PlayerStateFrequencyLimiter::UIMGReplicationGraphNode_PlayerStateFrequencyLimiter()
{
	bRequiresPrepareForReplicationCall = true;
}

void UIMGReplicationGraphNode_PlayerStateFrequencyLimiter::PrepareForReplication()
{
...
void UIMGReplicationGraphNode_PlayerStateFrequencyLimiter::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
...
void UIMGReplicationGraphNode_PlayerStateFrequencyLimiter::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	DebugInfo.Log(NodeName);
	DebugInfo.PushIndent();
...
void UIMGReplicationGraph::PrintRepNodePolicies()
{
	...
	GLog->Logf(TEXT("===================================="));
	GLog->Logf(TEXT("IMG Replication Routing Policies"));
	GLog->Logf(TEXT("===================================="));
...
FAutoConsoleCommandWithWorldAndArgs IMGPrintRepNodePoliciesCmd(TEXT("IMG.RepGraph.PrintRouting"),TEXT("Prints how actor classes are routed to RepGraph nodes"),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		for (TObjectIterator<UIMGReplicationGraph> It; It; ++It)
		{
...
FAutoConsoleCommandWithWorldAndArgs ChangeFrequencyBucketsCmd(TEXT("IMG.RepGraph.FrequencyBuckets"), TEXT("Resets frequency bucket count."), FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray< FString >& Args, UWorld* World)
{
	int32 Buckets = 1;
	if (Args.Num() > 0)
	{
	...
	UE_LOG(LogIMGRepGraph, Display, TEXT("Setting Frequency Buckets to %d"), Buckets);
