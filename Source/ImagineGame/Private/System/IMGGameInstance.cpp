...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGGameInstance)

namespace IMG
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarIMGTestEncryption(
		TEXT("IMG.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarIMGUseDTLSEncryption(
		TEXT("IMG.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using IMG.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarIMGTestDTLSFingerprint(
		TEXT("IMG.TestDTLSFingerprint"),
...
void UIMGGameInstance::Init()
{
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UIMGGameInstance::OnPreClientTravelToSession);
	}
}

void UIMGGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
AIMGPlayerController* UIMGGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AIMGPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UIMGGameInstance::CanJoinRequestedSession() const
{
...
void UIMGGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UIMGLocalPlayer* LocalPlayer = Cast<UIMGLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UIMGGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (IMG::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (IMG::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("IMGTest.pem");
...
void UIMGGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UIMGGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (IMG::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (IMG::bUseDTLSEncryption)
		{
