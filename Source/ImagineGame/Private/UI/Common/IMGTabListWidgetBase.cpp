...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGTabListWidgetBase)

void UIMGTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UIMGTabListWidgetBase::NativeConstruct()
{
...
void UIMGTabListWidgetBase::NativeDestruct()
{
	for (FIMGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UIMGTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FIMGTabDescriptor& OutTabInfo)
{
	const FIMGTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FIMGTabDescriptor& TabInfo) -> bool
	{
...
void UIMGTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FIMGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UIMGTabListWidgetBase::RegisterDynamicTab(const FIMGTabDescriptor& TabDescriptor)
{
...
void UIMGTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FIMGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UIMGTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UIMGTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FIMGTabDescriptor* TabInfoPtr = nullptr;
	
	FIMGTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UIMGTabButtonInterface::StaticClass()))
	{
		...
		{
			IIMGTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UIMGTabListWidgetBase::IsFirstTabActive() const
{
...
bool UIMGTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UIMGTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UIMGTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UIMGTabListWidgetBase::SetupTabs()
{
	for (FIMGTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
