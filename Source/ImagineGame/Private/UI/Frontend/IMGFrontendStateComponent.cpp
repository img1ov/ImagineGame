#include "UI/Frontend/IMGFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGFrontendStateComponent)
...
UIMGFrontendStateComponent::UIMGFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UIMGFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UIMGFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UIMGFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UIMGFrontendStateComponent::OnExperienceLoaded(const UIMGExperienceDefinition* Experience)
{
...
void UIMGFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UIMGFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UIMGFrontendStateComponent::OnUserInitialized);
	...
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		...
		TWeakObjectPtr<UIMGFrontendStateComponent> WeakThis = this;
...
void UIMGFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UIMGFrontendStateComponent::OnUserInitialized);
...
void UIMGFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UIMGFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
