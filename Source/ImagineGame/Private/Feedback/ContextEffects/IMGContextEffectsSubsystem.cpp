...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGContextEffectsSubsystem)
...
void UIMGContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UIMGContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UIMGContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UIMGContextEffectsLibrary* EffectLibrary : EffectsLibraries->IMGContextEffectsLibraries)
			{
...
bool UIMGContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UIMGContextEffectsSettings* ProjectSettings = GetDefault<UIMGContextEffectsSettings>())
	{
...
void UIMGContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UIMGContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UIMGContextEffectsSet* EffectsLibrariesSet = NewObject<UIMGContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UIMGContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UIMGContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->IMGContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UIMGContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
