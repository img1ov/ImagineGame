...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGContextEffectsLibrary)


void UIMGContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context,
...
void UIMGContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UIMGContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UIMGContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FIMGContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UIMGActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FIMGContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UIMGActiveContextEffects* NewActiveContextEffects = NewObject<UIMGActiveContextEffects>(this);
	...
	this->IMGContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UIMGContextEffectsLibrary::IMGContextEffectLibraryLoadingComplete(
	TArray<UIMGActiveContextEffects*> IMGActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(IMGActiveContextEffects);
}
