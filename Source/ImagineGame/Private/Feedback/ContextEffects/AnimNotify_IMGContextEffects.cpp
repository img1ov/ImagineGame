...
#include "Feedback/ContextEffects/IMGContextEffectsInterface.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_IMGContextEffects)



UAnimNotify_IMGContextEffects::UAnimNotify_IMGContextEffects()
{
}

void UAnimNotify_IMGContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_IMGContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_IMGContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_IMGContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> IMGContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UIMGContextEffectsInterface>())
			{
				// If so, add it to the Array
				IMGContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UIMGContextEffectsInterface>())
					{
						IMGContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* IMGContextEffectImplementingObject : IMGContextEffectImplementingObjects)
			{
				if (IMGContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IIMGContextEffectsInterface::Execute_AnimMotionEffect(IMGContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UIMGContextEffectLibrary type
						if (UIMGContextEffectsLibrary* EffectLibrary = Cast<UIMGContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_IMGContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_IMGContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FIMGContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FIMGContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FIMGContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
