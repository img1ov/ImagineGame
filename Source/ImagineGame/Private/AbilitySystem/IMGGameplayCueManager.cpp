...
#include UE_INLINE_GENERATED_CPP_BY_NAME(IMGGameplayCueManager)

enum class EIMGEditorLoadMode
{
...
namespace IMGGameplayCueManagerCvars
{
	...
	static EIMGEditorLoadMode LoadMode = EIMGEditorLoadMode::LoadUpfront;
}
...
UIMGGameplayCueManager::UIMGGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UIMGGameplayCueManager* UIMGGameplayCueManager::Get()
{
	return Cast<UIMGGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UIMGGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UIMGGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogIMG, Warning, TEXT("UIMGGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UIMGGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (IMGGameplayCueManagerCvars::LoadMode)
	{
	case EIMGEditorLoadMode::LoadUpfront:
		return true;
	case EIMGEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EIMGEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UIMGGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UIMGGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UIMGGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UIMGGameplayCueManager* GCM = Cast<UIMGGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogIMG, Error, TEXT("DumpGameplayCues failed. No UIMGGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogIMG, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogIMG, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogIMG, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogIMG, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogIMG, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogIMG, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogIMG, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogIMG, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogIMG, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogIMG, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogIMG, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogIMG, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UIMGGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UIMGGameplayCueManager* StrongThis = Get())
					{
...
void UIMGGameplayCueManager::HandlePostGarbageCollect()
{
...
void UIMGGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogIMG, Warning, TEXT("UIMGGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UIMGGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (IMGGameplayCueManagerCvars::LoadMode)
	{
	case EIMGEditorLoadMode::LoadUpfront:
...
void UIMGGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UIMGGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UIMGGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UIMGGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UIMGGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UIMGGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
