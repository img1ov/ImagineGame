...
#include "IMGReplaySubsystem.generated.h"
...
class UIMGReplayListEntry : public UObject
{
	GENERATED_BODY()
...
UCLASS(MinimalAPI, BlueprintType)
class UIMGReplayList : public UObject
{
	GENERATED_BODY()

public:
	UPROPERTY(BlueprintReadWrite, Category=Replays)
	TArray<TObjectPtr<UIMGReplayListEntry>> Results;
};

/** Subsystem to handle recording/loading replays */
UCLASS(MinimalAPI)
class UIMGReplaySubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

public:
	UE_API UIMGReplaySubsystem();
	...
	UE_API void PlayReplay(UIMGReplayListEntry* Replay);
