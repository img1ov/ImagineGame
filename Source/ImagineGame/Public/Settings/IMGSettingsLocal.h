...
#include "IMGSettingsLocal.generated.h"
...
USTRUCT()
struct FIMGScalabilitySnapshot
{
	GENERATED_BODY()

	FIMGScalabilitySnapshot();
 ...
 */
UCLASS()
class UIMGSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EIMGStatDisplayMode GetPerfStatDisplayState(EIMGDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EIMGDisplayablePerformanceStat Stat, EIMGStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UIMGSettingsLocal, FPerfStatSettingsChanged);
	...
	DECLARE_EVENT(UIMGSettingsLocal, FLatencyFlashInidicatorSettingChanged);
	UFUNCTION()
	...
	DECLARE_EVENT(UIMGSettingsLocal, FLatencyStatEnabledSettingChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EIMGDisplayablePerformanceStat, EIMGStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FIMGScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FIMGScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UIMGSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
