...
#include "IMGSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EIMGAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM()
enum class EIMGGamepadInputAPIOption : uint8
{
...
/**
 * UIMGSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UIMGSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UIMGSettingsShared, FOnSettingChangedEvent, UIMGSettingsShared* Settings);
...
public:
	...
	static UIMGSettingsShared* CreateTemporarySettings(const UIMGLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UIMGSettingsShared* LoadOrCreateSettings(const UIMGLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UIMGSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UIMGLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
private:
	...
	UPROPERTY()
	EIMGGamepadInputAPIOption GamepadInputAPIOptions;

public:
	UFUNCTION()
	EIMGGamepadInputAPIOption GetGamepadInputAPIOption() const { return GamepadInputAPIOptions; }
	UFUNCTION()
	void SetGamepadInputAPIOption(const EIMGGamepadInputAPIOption NewValue);
...
public:
	UFUNCTION()
	EIMGAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EIMGAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EIMGAllowBackgroundAudioSetting AllowAudioInBackground = EIMGAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EIMGGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EIMGGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EIMGGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EIMGGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EIMGGamepadSensitivity GamepadLookSensitivityPreset = EIMGGamepadSensitivity::Normal;
	UPROPERTY()
	EIMGGamepadSensitivity GamepadTargetingSensitivityPreset = EIMGGamepadSensitivity::Normal;
