...
class IIMGTeamAgentInterface;
class UIMGTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UIMGTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements IIMGTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UIMGTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UIMGTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<IIMGTeamAgentInterface> TeamInterfacePtr;
