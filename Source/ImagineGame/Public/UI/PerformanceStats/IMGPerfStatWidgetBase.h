...
#include "IMGPerfStatWidgetBase.generated.h"
...
class SIMGLatencyGraph : public SLeafWidget
{
public:
	/** Begin the arguments for this slate widget */
	SLATE_BEGIN_ARGS(SIMGLatencyGraph)
		: _DesiredSize(150, 50),
...
class UIMGPerfStatGraph : public UUserWidget
{
	GENERATED_BODY()

public:
	UIMGPerfStatGraph(const FObjectInitializer& ObjectInitializer);
...
protected:
	...
	TSharedPtr<SIMGLatencyGraph> SlateLatencyGraph;
};
 ...
 */
 UCLASS(Abstract)
class UIMGPerfStatWidgetBase : public UCommonUserWidget
{
public:
	GENERATED_BODY()
...
public:
	...
	EIMGDisplayablePerformanceStat GetStatToDisplay() const
	{
...
protected:
 	...
 	UIMGPerformanceStatSubsystem* GetStatSubsystem();
	...
	UPROPERTY(BlueprintReadWrite, meta=(BindWidget, OptionalWidget=true))
 	TObjectPtr<UIMGPerfStatGraph> PerfStatGraph;
 	
	// Cached subsystem pointer
	UPROPERTY(Transient)
	TObjectPtr<UIMGPerformanceStatSubsystem> CachedStatSubsystem;
