...
#include "IMGInputUserSettings.generated.h"

#define UE_API IMAGINEGAME_API

/**
 * Custom settings class for any input related settings for the IMG game.
 * This will be serialized out at the same time as the IMG Shared Settings and is
 * compatible with cloud saves through by calling the "Serialize" function.
 */
UCLASS(MinimalAPI)
class UIMGInputUserSettings : public UEnhancedInputUserSettings
{
	GENERATED_BODY()
public:
 ...
 */
UCLASS(MinimalAPI)
class UIMGPlayerMappableKeySettings : public UPlayerMappableKeySettings
{
	GENERATED_BODY()
