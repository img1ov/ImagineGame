...
#include "IMGPerformanceStatTypes.h"
#include "Algo/MaxElement.h"
#include "Algo/MinElement.h"
#include "Stats/StatsData.h"
#include "Subsystems/GameInstanceSubsystem.h"

#include "IMGPerformanceStatSubsystem.generated.h"

enum class EIMGDisplayablePerformanceStat : uint8;

class FSubsystemCollectionBase;
class UIMGPerformanceStatSubsystem;
class UObject;
struct FFrame;
...
struct FIMGPerformanceStatCache : public IPerformanceDataConsumer
{
public:
	FIMGPerformanceStatCache(UIMGPerformanceStatSubsystem* InSubsystem)
	...
	*/
	double GetCachedStat(EIMGDisplayablePerformanceStat Stat) const;
	 ...
	 */
	const FSampledStatCache* GetCachedStatData(const EIMGDisplayablePerformanceStat Stat) const;

protected:

	void RecordStat(const EIMGDisplayablePerformanceStat Stat, const double Value);
	
	UIMGPerformanceStatSubsystem* MySubsystem;

	/**
	 * Caches the sampled data for each of the performance stats currently available
	 */
	TMap<EIMGDisplayablePerformanceStat, FSampledStatCache> PerfStateCache;
};
...
class UIMGPerformanceStatSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	TSharedPtr<FIMGPerformanceStatCache> Tracker;
};
