...
#include "IMGAudioSettings.generated.h"
...
USTRUCT()
struct FIMGSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 *
 */
UCLASS(MinimalAPI, config = Game, defaultconfig, meta = (DisplayName = "IMGAudioSettings"))
class UIMGAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FIMGSubmixEffectChainMap> HDRAudioSubmixEffectChain;

	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FIMGSubmixEffectChainMap> LDRAudioSubmixEffectChain;
