...
#include "IMGGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UIMGRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EIMGAbilityTargetingSource : uint8
{
...
/**
 * UIMGGameplayAbility_RangedWeapon
 *
...
public:
	...
	UIMGGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="IMG|Ability")
	UIMGRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UIMGRangedWeaponInstance* WeaponData = nullptr;
